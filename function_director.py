...
def insert_employ_screen():
    ...
    l1 = Label(root_func, text='id сотрудника').grid(row=1, column=0)
    l2 = Label(root_func, text='ФИО сотрудника').grid(row=2, column=0)
    l3 = Label(root_func, text='Документ').grid(row=3, column=0)
    l4 = Label(root_func, text='Должность').grid(row=4, column=0)
    l5 = Label(root_func, text='Магазин').grid(row=5, column=0)

    e = [None for i in range(5)]
    for i in range(0, 5):
        e[i] = Entry(root_func)
    b0 = Button(root_func, text='Добавить', command=insert_employ_sql).grid(row=6, column=1)
...
def delete_employ_screen():
    ...
    for i in range(0, 1):
        e[i] = Entry(root_func)
    b0 = Button(root_func, text='Уволить', command=delete_employ_sql).grid(row=2, column=1)
...
def update_contract_screen():
    ...
    l5 = Label(root_func, text='id продукта').grid(row=5, column=0)
    l6 = Label(root_func, text='Статус оплаты(1-опл, 0-нет)').grid(row=6, column=0)
    l7 = Label(root_func, text='Статус выполнения(1-вып, 0-нет)').grid(row=7, column=0)
    l8 = Label(root_func, text='Магазин').grid(row=8, column=0)
    e = [None for i in range(8)]
    for i in range(0, 8):
        e[i] = Entry(root_func)
    b0 = Button(root_func, text='Обновить', command=update_contract_sql).grid(row=9, column=1)
...
def contract_all_info_screen_sql():
    ...
    table = ttk.Treeview(cont_root, columns=columns, show='headings')
